...
describe('mapSchedulingMoment(...)', (): void => {
  ...
  const testData = [
    ...
    },
    {
      expected: {
        ...
        hasError: false,
        history: [],
        schedulingCompletedOn: completedOn,
      },
      status: 'SCHEDULED',
  ...
  testData.forEach((test): void => {
    it(`${test.expected.schedulingCompletedOn ? 'should': 'should not'} set the completion date when the status is ${test.status}`, (): void => {
      const request = generate.orderRequest();

      request.orderRequestStatusUpdates = test.statusUpdates;

      const result = mapSchedulingMoment(request);

      expect(result.schedulingCompletedOn).toEqual(test.expected.schedulingCompletedOn);
    });
